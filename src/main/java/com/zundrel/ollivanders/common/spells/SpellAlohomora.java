...
public class SpellAlohomora extends Spell implements IRaytraceSpell {
    public SpellAlohomora(String name, String verbalName, EnumSpellCategory spellCategory) {
    ...
    public void onHitBlock(int powerLevel, World world, PlayerEntity player, BlockHitResult rayTraceResult) {
        ...
        if (block instanceof DoorBlock && !state.get(Properties.OPEN)) {
            ...
            doorBlock.setOpen(world, rayTraceResult.getBlockPos(), true);
            ...
            int i = state.getMaterial() == Material.METAL ? 1037 : 1007;
